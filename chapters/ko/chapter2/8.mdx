...
<Question
	choices={[
		...
		{
			text: "토크나이저가 텍스트를 처리하고 ID를 반환합니다. 모델은 ID를 이용해 예측 결과를 출력합니다. 그리고 토크나이저를 다시 사용해 예측 결과를 일종 텍스트로 변환할 수 있습니다.",
...
### 2. 기본 Transformer 모델에 의해 만들어지는 출력 텐서는 몇 차원이며, 각 텐서가 무엇을 의미하나요?
...
<Question
	choices={[
		...
		{
			text: "2: 시퀀스 길이, 은닉 크기",
			explain: "틀렸습니다! 모든 Transformer 모델은 단일 시퀀스의 경우 크기가 1인 배치로 처리합니다!"
		},
		{
			text: "3: 시퀀스 길이, 배치 크기, 은닉 크기",
			explain: "정답!",
            correct: true
		}
	]}
...
<Question
	choices={[
		...
		},
		{
			...
			explain: "정확합니다. <code>AutoModel</code>은 적절한 모델 구조를 초기화하기 위해 체크포인트만을 필요로 합니다.",
			correct: true
		},
		{
...
<Question
	choices={[
		{
			text: "소프트맥스 함수는 더 신뢰성 있는 로깃 결과를 만들기 위해 정규화를 수행합니다.",
			explain: "오답입니다, 소프트맥스 함수는 결과의 신뢰도에 영향을 주지 않습니다."
		},
		{
			text: "소프트맥스 함수는 상한과 하한을 적용해서 로깃 결과를 이해하기 쉽게 만듭니다.",
			explain: "정답입니다! 소프트맥스 함수를 거친 로깃 값은 0과 1 사이 값으로 바뀌게 됩니다. 하지만 이 이유가 소프트맥스 함수를 사용하는 유일한 이유는 아닙니다.",
            correct: true
		},
		{
